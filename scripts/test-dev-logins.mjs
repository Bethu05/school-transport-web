...
const accounts = [
...

    {
        label: 'Driver',
        email:
            process.env.VITE_DEV_DRIVER_EMAIL,
...
async function testAccount(account) {
    if (!account.email) {
        throw new Error(
            `${account.label} email is missing`,
        );
    }
    ...
    if (!login.accessToken) {
        throw new Error(
            `${account.label} returned no access token`,
        );
    }
    ...
    if (
        !account.expectedRoles.includes(role)
    ) {
        throw new Error(
