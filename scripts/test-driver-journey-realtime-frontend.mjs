...
assert(
  progress.includes("subscribeToTrip("),
  "Driver Journey explicitly subscribes to its assigned trip",
);

assert(
  progress.includes("assignedTripId") &&
    progress.includes("progressQuery.data"),
...
const gpsHandlerStart = progress.indexOf("function handleVehicleLocation");

const gpsHandlerEnd = progress.indexOf(
  "function handleStopEvent",
  gpsHandlerStart,
);

const gpsHandler = progress.slice(gpsHandlerStart, gpsHandlerEnd);

assert(
  gpsHandlerStart >= 0 && !gpsHandler.includes("invalidateQueries"),
  "GPS packets update local state without HTTP refetch",
);

assert(
  progress.includes("<TrackingProgressPanel"),
  "Driver Journey renders realtime next-stop ETA",
);

assert(
  progress.includes("<LiveTrackingMap"),
  "Driver Journey renders realtime vehicle map",
);

assert(
  progress.includes("activeLiveLocation"),
  "Driver Journey protects against stale telemetry from an old trip",
);

assert(
  progress.includes("unsubscribeFromTrip("),
  "Driver leaves the trip room during cleanup",
);

assert(
  progress.includes("socket.disconnect()"),
