...
for (
  ...
  if (
    ...
    throw new Error(
      `Cross-tenant driver returned: ${driver.id}`,
    );
  }
  ...
  if (
    typeof driver.id !==
      'string' ||
    typeof driver.firstName !==
      'string' ||
    typeof driver.lastName !==
      'string' ||
    typeof driver.licenseNumber !==
      'string' ||
    typeof driver.status !==
      'string'
  ) {
    throw new Error(
...
console.log(
  '✓ driver field contract',
);

console.log(
  `✓ ${drivers.length} driver(s) returned`,
);

console.log();

console.log(
  'Drivers API checkpoint PASSED',
);
