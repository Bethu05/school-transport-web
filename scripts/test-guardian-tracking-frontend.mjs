...
const api = read("src/tracking/guardian-tracking.api.ts");

const realtime = read("src/tracking/tracking.realtime.ts");

const panel = read("src/tracking/GuardianTrackingPanel.tsx");

const page = read("src/tracking/TrackingPage.tsx");

console.log();
console.log("Guardian Tracking frontend checkpoint");
...
check(
  panel.includes("location.tripId"),
  "guardian realtime feed filtered by authorised trip",
);

check(
  panel.includes("child.firstName") && panel.includes("child.lastName"),
  "child name shown instead of student UUID",
);

const rendersRawTripId = />\s*\{\s*tripId\s*\}\s*</m.test(panel);
...
console.log();
console.log("Guardian Tracking frontend checkpoint PASSED");
