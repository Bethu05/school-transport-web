...
assert(
  ADMIN_EMAIL,
  'VITE_DEV_ADMIN_EMAIL is missing',
);
...
assert(
  login?.accessToken,
  'Administrator login returned no access token',
);
...
const email =
  `guardian-checkpoint-${unique}@example.com`;

const updatedEmail =
  `guardian-updated-${unique}@example.com`;
...
assert(
  created?.id,
  'Guardian create returned no id',
);

assert(
  created.status ===
    'active',
  'New Guardian was not active',
);

assert(
  created.email ===
    email,
  'Created Guardian email mismatch',
);
...
const listAfterCreate =
  await request(
    `/guardians?page=1&limit=10&search=${encodeURIComponent(
      email,
    )}`,
    {
      token,
...
assert(
  Array.isArray(
    listAfterCreate?.items,
  ),
  'Guardian list response has no items array',
);

assert(
  listAfterCreate.items.some(
    (guardian) =>
      guardian.id ===
      created.id,
  ),
...
assert(
  updated.firstName ===
    'Updated',
  'Guardian first name update failed',
);

assert(
  updated.email ===
    updatedEmail,
  'Guardian email update failed',
);

assert(
  updated.notifyBoarded ===
    false,
  'notifyBoarded update failed',
);

assert(
  updated.notifyTripUpdates ===
    false,
  'notifyTripUpdates update failed',
);
...
console.log(
  '✓ reactivation persisted',
);

console.log('');
console.log(
  'Guardians CRUD API checkpoint PASSED',
);
