...
const form = read("src/incidents/IncidentFormDialog.tsx");
...
console.log();
console.log("Incidents frontend checkpoint");
console.log("-----------------------------");
...
assert(
  page.includes("FRONTEND_PERMISSIONS.INCIDENTS_UPDATE"),
...
assert(
  form.includes('value="resolved"') &&
    form.includes('value="closed"') &&
    form.includes('value="open"'),
  "incident lifecycle supports resolve close and reopen",
);

assert(
  !page.includes("deleteIncident("),
  "frontend does not invent unsupported hard delete",
);

assert(app.includes("<IncidentsPage />"), "Incidents page connected to router");

console.log();
console.log("Incidents frontend checkpoint PASSED");
