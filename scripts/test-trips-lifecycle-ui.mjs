...
const page = readFileSync("src/trips/TripsPage.tsx", "utf8");
...
assert(page.includes("boardTrip("), "Board lifecycle API is wired");

assert(page.includes("startTrip("), "Start lifecycle API is wired");

assert(page.includes("completeTrip("), "Complete lifecycle API is wired");
...
assert(
  page.includes("FRONTEND_PERMISSIONS.TRIPS_CANCEL"),
  "Cancel action uses backend permission",
);

console.log("");
console.log("Trips lifecycle UI checkpoint PASSED");
