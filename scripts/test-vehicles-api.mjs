...
console.log("Vehicles API checkpoint");
 ...
 * vehicles endpoint.
 */
const vehiclesResponse = await fetch(`${apiUrl}/vehicles?page=1&limit=100`, {
  headers: {
...
const vehicles = await readJson(vehiclesResponse);

/**
 * 3. Verify the backend pagination
 * contract expected by the frontend.
 */
if (!vehicles || !Array.isArray(vehicles.items)) {
  throw new Error("Vehicles response items must be an array");
}

for (const field of ["page", "limit", "total", "totalPages"]) {
  if (typeof vehicles[field] !== "number") {
    throw new Error(`Vehicles response ${field} must be a number`);
  }
}

console.log("✓ pagination contract");
...
for (const vehicle of vehicles.items) {
  if (vehicle.tenantId !== tenantId) {
    throw new Error(`Cross-tenant vehicle returned: ${vehicle.id}`);
  }
...
console.log(`✓ ${vehicles.items.length} vehicle(s) returned`);

console.log(`✓ ${vehicles.total} vehicle(s) total`);

console.log();
console.log("Vehicles API checkpoint PASSED");
