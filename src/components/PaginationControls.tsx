...
function rangeStart(page: number, limit: number, total: number): number {
  if (total === 0) {
    return 0;
  }
...
function rangeEnd(page: number, limit: number, total: number): number {
  if (total === 0) {
    return 0;
  }
...
export function PaginationControls({
  ...
  function handleLimitChange(event: SelectChangeEvent<number>): void {
  ...
  return (
    ...
    >
      ...
      >
        Showing <strong>{rangeStart(page, limit, total)}</strong>
        {"–"}
        <strong>{rangeEnd(page, limit, total)}</strong> of{" "}
        <strong>{total}</strong>
      </Typography>
