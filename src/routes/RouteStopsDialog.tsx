...
function parseOptionalOffset(value: string): number | null | "invalid" {
  if (!value.trim()) {
    return null;
  }
...
export function RouteStopsDialog({
  ...
  const routeStopsQuery = useQuery({
    ...
    enabled: Boolean(open && tenantId && routeId),
    ...
    queryFn: async () => {
      ...
      return listRouteStops(tenantId, routeId);
    },
  });
  ...
  const routeStops = routeStopsQuery.data ?? EMPTY_ROUTE_STOPS;

  const stops = stopsQuery.data ?? EMPTY_STOPS;
  ...
  const availableStops = useMemo(() => {
    ...
    const attachedStopIds = new Set(
      routeStops.map((routeStop) => routeStop.stopId),
    );
    ...
    return stops
      .filter(
        (stop) =>
          ...
          (stop.schoolId === null || stop.schoolId === route.schoolId),
      )
  ...
  function openEditStop(routeStop: RouteStop): void {
    setFormError(null);
  ...
  const addMutation = useMutation({
    mutationFn: async (input: AddRouteStopInput) => {
      if (!tenantId || !routeId) {
        throw new Error("Route context is unavailable");
      }

      return addRouteStop(tenantId, routeId, input);
    },
  ...
  const updateMutation = useMutation({
    mutationFn: async ({
      ...
      return updateRouteStop(tenantId, routeId, routeStopId, input);
    },
  ...
  const removeMutation = useMutation({
    mutationFn: async (routeStopId: string) => {
      ...
      return removeRouteStop(tenantId, routeId, routeStopId);
    },
  ...
  async function submitAdd(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    const stopOrder = parsePositiveInteger(addForm.stopOrder);

    const offset = parseOptionalOffset(addForm.plannedOffsetMinutes);
    ...
    await addMutation.mutateAsync({
      ...
      stopOrder,
      plannedOffsetMinutes: offset === null ? undefined : offset,
    });
  }
  ...
  async function submitEdit(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    const stopOrder = parsePositiveInteger(editForm.stopOrder);

    const offset = parseOptionalOffset(editForm.plannedOffsetMinutes);
  ...
  const routeInactive = route?.status === "inactive";
    ...
    <>
      ...
      >
        <DialogTitle sx={{ fontWeight: 850 }}>
          {route ? `Route stops — ${route.name}` : "Route stops"}
        </DialogTitle>
        ...
        <DialogContent>
          ...
          {stopsQuery.isError ? (
            <Alert severity="warning" sx={{ mb: 2 }}>
              Available stops could not be loaded. Existing route stops can
              still be viewed.
            </Alert>
          ) : null}
          ...
          >
            <Box>
              <Typography sx={{ fontWeight: 850 }}>Ordered stops</Typography>
          ...
          stops.length > 0 ? (
            ...
            >
              Every active stop available to this school is already on the
              route.
            </Typography>
          ) : null}
        </DialogContent>
      ...
      >
        <Box component="form" onSubmit={submitAdd}>
          <DialogTitle sx={{ fontWeight: 850 }}>Add stop to route</DialogTitle>
      ...
      <Dialog
        ...
        onClose={
          updateMutation.isPending ? undefined : () => setEditingStop(null)
        }
        fullWidth
        maxWidth="sm"
      >
        <Box component="form" onSubmit={submitEdit}>
          <DialogTitle sx={{ fontWeight: 850 }}>Edit route stop</DialogTitle>
      ...
      <Dialog
        ...
        onClose={
          removeMutation.isPending ? undefined : () => setRemoveTarget(null)
        }
        fullWidth
        maxWidth="xs"
      >
        ...
        <DialogActions sx={{ px: 3, pb: 3 }}>
          ...
          >
            {removeMutation.isPending ? "Removing..." : "Remove stop"}
          </Button>
        </DialogActions>
      </Dialog>
    </>
