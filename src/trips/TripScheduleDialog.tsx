...
import type { CreateTripInput } from "./trips.api";
...
interface TripScheduleDialogProps {
   ...
   */
  onSchedule: (input: CreateTripInput) => Promise<void>;
}
 ...
 */
function toBusinessIso(serviceDate: string, time: string): string {
  return `${serviceDate}` + `T${time}:00` + BUSINESS_UTC_OFFSET;
}
...
function routeLabel(route: Route): string {
  ...
  const type =
    route.routeType === "pickup"
      ? "Pickup"
      : route.routeType === "dropoff"
        ? "Drop-off"
        : "Other";

  return `${route.name}${code}` + ` — ${type}`;
}
...
export function TripScheduleDialog({
  ...
  const [form, setForm] = useState<ScheduleFormState>(() => ({
    routeId: eligibleRoutes.length === 1 ? eligibleRoutes[0].id : "",

    serviceDate: businessDateToday(),
  ...
  const selectedRoute =
    eligibleRoutes.find((route) => route.id === form.routeId) ?? null;
  ...
  function updateField<K extends keyof ScheduleFormState>(
    key: K,
  ...
  function selectRoute(routeId: string): void {
    const route = eligibleRoutes.find((item) => item.id === routeId) ?? null;

    setForm((current) => {
      const vehicle = vehicles.find((item) => item.id === current.vehicleId);

      const driver = drivers.find((item) => item.id === current.driverId);

      const vehicleStillValid =
        !route ||
        !vehicle ||
        vehicle.schoolId === null ||
        vehicle.schoolId === route.schoolId;

      const driverStillValid =
        !route ||
        !driver ||
        driver.schoolId === null ||
        driver.schoolId === route.schoolId;
      ...
      return {
        ...
        vehicleId: vehicleStillValid ? current.vehicleId : "",

        driverId: driverStillValid ? current.driverId : "",
      };
    });
  }
  ...
  async function handleSubmit(
    ...
    if (!form.serviceDate) {
      setValidationError("Please select the service date.");
    ...
    if (!form.startTime) {
      setValidationError("Please enter the departure time.");

      return;
    }

    if (!form.endTime) {
      setValidationError("Please enter the expected finish time.");
    ...
    const scheduledStartAt = toBusinessIso(form.serviceDate, form.startTime);

    const scheduledEndAt = toBusinessIso(form.serviceDate, form.endTime);
        ...
        <DialogContent>
          ...
          >
            ...
            <TextField
              ...
              label="Route"
              value={form.routeId}
              disabled={saving || eligibleRoutes.length === 0}
